...
// Сложность алгоритма:
// Вставка узла (метод add): В среднем случае, время O(log N),
// где N - количество узлов в дереве. В худшем случае (например, если узлы добавляются в упорядоченном порядке),
// время может быть O(N), что приводит к несбалансированному дереву.
// Вывод всех узлов в порядке обхода (метод print):
